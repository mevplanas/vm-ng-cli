...
export class SidebarKindergartensComponent implements OnInit {
  ...
  async createBuffer(options: any, point) {
    // Initialize required options for buffer execution
    ...
    try {
      // Execute buffer operation and wait for results
      const results = await this.geometryService.buffer(parameters);

      // Assuming results are successfully returneds
