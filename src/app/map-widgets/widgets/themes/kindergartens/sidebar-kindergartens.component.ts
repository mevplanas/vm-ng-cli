...
export class SidebarKindergartensComponent implements OnInit {
  ...
  ngOnInit() {
    this.geometryService = this.menuToolsService.addGeometryService(this.config.mapOptions.staticServices.geometryUrl);

    this.simpleQuery(this.config.themes.kindergartens.layers.darzeliai.dynimacLayerUrls + '/3').then(features => {
  ...
  selectGartens(id: number, domElement) {
    ...
    featureLayer.queryFeatures(query).then((results) => {
      ...
      const groupFeatureSelectionLayer = this.mapService.initFeatureSelectionGraphicLayer('FeatureSelection',
      features.layer.maxScale, features.layer.minScale, 'hide');
