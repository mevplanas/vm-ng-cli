...
export class BuildingsComponent implements AfterViewInit, OnDestroy {
  ...
  hitTestFeaturePopup(view: any, event: any) {
    ...
    view.hitTest(screenPoint).then((features) => {
      ...
      if (
        features.results.length !== 1 &&
        features.results[0].graphic.attributes.ADRESAS
      ) {
  ...
  addSelection(results, outlineColor = [181, 14, 18, 1]): void {
    ...
    const groupFeatureSelectionLayer =
      this.mapService.initFeatureSelectionGraphicLayer(
        "FeatureSelection",
        results.layer.maxScale,
        results.layer.minScale,
        "hide"
      );
  ...
  ngAfterViewInit() {
    ...
    if (snapshotUrl) {
      this.buildingsLayersService.addCustomLayers(
        this.queryParams,
        snapshotUrl
      );
    }
