...
import { BasemapsService } from './widgets/basemaps.service';
...
export class ShareButtonService {
  ...
  getVisibleLayersIds(view, custom = false) {
    // ids will have 2 properties: 'identificationsIds' (layers to be identified)
    // and 'visibilityIds' (all visible layers that must be checked and visible depending on mxd settings or user activated layers)
  ...
  iterateVisibleLayersIds(item, viewScale) {
    // small fix: add layer id that doen't exist, for example 999,
    // in order to prevent all layers identification when all lists are turned off
    ...
    if ((item.visible) && (!item.layer.isRaster) && (item.layer.sublayers)) {
      ...
      const subLayers = item.layer.sublayers.items;
      subLayers.map((subLayer) => {
        let minScale = subLayer.minScale;
        const maxScale = subLayer.maxScale;
        ...
        if (subLayer.visible) {
          ...
          if ((maxScale < viewScale) && (viewScale < minScale)) {
            ...
            if (subLayer.sublayers) {
              // 3 layer if exist
              const subsublayers = subLayer.sublayers.items;
              subsublayers.map(subsublayer => {
                let subMinScale = subsublayer.minScale;
                const subMaxScale = subsublayer.maxScale;
                ...
                if (subsublayer.visible) {
                  ...
                  if ((subMaxScale < viewScale) && (viewScale < subMinScale)) {
                    ...
                    if (subsublayer.sublayers) {
                      // 4 layer if exist
                      const subsubsublayers = subsublayer.sublayers.items;
                      subsubsublayers.map(subsubsublayer => {
                        let subMinScale = subsubsublayer.minScale;
                        const subMaxScale = subsubsublayer.maxScale;
                        ((subMinScale === 0)) ? subMinScale = 99999999 : subMinScale;
                        // if layer is visible and in view scale
  ...
  getVisibleSubLayerNumber(view: any, custom = false) {
    ...
    const ids: any = !custom ? this.getVisibleLayersIds(view) : this.getVisibleLayersIds(view, true);
    // tslint:disable-next-line: max-line-length
